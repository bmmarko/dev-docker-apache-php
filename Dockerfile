...
RUN apt-get install -y $(cat /.buildDeps) --no-install-recommends \
    ...
    && cd /usr/src/php/ext/ \
    && curl -L http://xdebug.org/files/xdebug-2.7.0.tgz | tar -zxf - \
    && mv xdebug-2.7.0 xdebug \
...
RUN docker-php-ext-install $(cat /.phpModules)
...
RUN apt-get install -y libmagickcore-6.q16-3 \
...
RUN apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false -o APT::AutoRemove::SuggestsImportant=false $(cat /.buildDeps) \
